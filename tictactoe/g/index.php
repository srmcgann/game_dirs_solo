          ...
          drawBoard=()=>{
            ...
            if(Players.length){
              if(!moves){
                if(lastWinnerWasOp !=-1){
                  if(lastWinnerWasOp){
                    x.fillText(Players.filter(v=>+v.playerData.id == +gmid)[0].playerData.name + ` goes first as X. waiting...`,20,40)
                  }else{
                    x.fillText(Players.filter(v=>+v.playerData.id != +gmid)[0].playerData.name + ` goes first as X. waiting...`,20,40)
                  }
                }else{
                ...
                if(!((turnID+moves+(lastWinnerWasOp==-1?0:(lastWinnerWasOp?1:0)))%2)){
                  if(inplay){
                    x.fillText(`your move...`, 20, 40)
                  }else{
                    x.fillText(`game over! ${lastWinnerWasOp?OPname:challengerName} won...`,20,40)                  }
                }else{
                  if(inplay){
                    x.fillText(`waiting for ${(moves+(opIsX?0:1))%2?challengerName:OPname} to make a move...`,20,40)
                  }else{
                    x.fillText(`game over! ${lastWinnerWasOp?OPname:challengerName} won...`,20,40)                  }
                }
              }
